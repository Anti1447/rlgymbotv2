...
class PlayerData(object):
    def __init__(self):
        ...
        self.air_time_since_jump = 0.0  # Track air time since jump
        self.is_boosting: bool = False  # New attribute to track boosting
        self._previous_boost_amount: float = -1  # Internal attribute to track boost usage
    ...
    def copy(self, other):
        ...
        self.air_time_since_jump = other.air_time_since_jump
        self.is_boosting = other.is_boosting  # Copy boosting state
        self._previous_boost_amount = other._previous_boost_amount  # Copy previous boost amount
    ...
    def update_air_time(self, delta_time):
        if not self.on_ground and self.has_jump:
            self.air_time_since_jump += delta_time
        else:
            self.air_time_since_jump = 0.0

    def update_boosting(self):
        """
        Updates the is_boosting attribute based on changes in boost_amount.
        """
        if self._previous_boost_amount >= 0:  # Ensure we have a valid previous value
            self.is_boosting = self.boost_amount < self._previous_boost_amount
        else:
            self.is_boosting = False  # Default to not boosting on the first frame

        # Update the previous boost amount for the next tick
        self._previous_boost_amount = self.boost_amount
